...
export function EmployeeProvider({ children }) {
  ...
  const getEmployee = useCallback(
    (id) => employees.find((e) => e.id === parseInt(id)),
    [employees]
  );
  ...
  const departmentData = [...new Set(employees.map((e) => e.department))].map(
    (dept) => {
      ...
      return {
        ...
        avgSalary: Math.round(group.reduce((acc, e) => acc + e.salary, 0) / group.length),
      };
    }
  );
  ...
  return (
    <EmployeeContext.Provider
      value={{ employees, addEmployee, updateEmployee, deleteEmployee, getEmployee, stats, departmentData, notification }}
    >
      {children}
    </EmployeeContext.Provider>
  );
